...
const ModalObservacion = ({ isOpen, onClose, onSave }) => {
  ...
  // Reset observation when modal opens or closes
  useEffect(() => {
    if (!isOpen) {
      setObservacion('');
    }
  ...
  const handleSave = () => {
    ...
    onSave(observacion);
    // No need to call onClose here since onSave will trigger a re-render
  };

  const handleCancel = () => {
    // Clear the observation and close the modal
    setObservacion('');
    onClose();
  };
    ...
    <div className="fixed z-50 inset-0 bg-black bg-opacity-50 flex justify-center items-center px-4">
      <div className="bg-white p-6 rounded-lg w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl relative">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-4">
          ...
          <button
            className="w-full sm:w-auto bg-white hover:bg-[#3B82F6] text-green-600 rounded-lg border-2 border-green-600 px-4 py-2 hover:text-white transition duration-300 text-sm sm:text-base"
            onClick={handleSave}
          >
            Guardar
          </button>
